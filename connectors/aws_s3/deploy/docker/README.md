...
This guide shows how to run the AWS S3 connector with Docker Compose.

## Prerequisites
- AWS credentials (env vars or mounted files) with permissions to list/read (and optionally write/move/delete) objects.
...
Run:
```bash
docker compose -f deploy/docker/docker-compose-aws-s3-connector.yaml up -d
```
