...
def prepare(c):
    ...
    c.run(f"mkdir -p {export_folder}/connectors/google_cloud_storage")
    c.run(f"mkdir -p {export_folder}/dsx_connect/models")
    c.run(f"mkdir -p {export_folder}/dsx_connect/utils")

    # Copy connectors
    c.run(f"rsync -av --exclude '__pycache__' {project_root_dir}/connectors/google_cloud_storage/ {export_folder}/connectors/google_cloud_storage/ --exclude 'deploy' --exclude 'dist' --exclude 'tasks.py'")
