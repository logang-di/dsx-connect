...
def prepare(c):
    ...
    c.run(f"mkdir -p {export_folder}/connectors/sharepoint")
    c.run(f"mkdir -p {export_folder}/dsx_connect/models")
    c.run(f"mkdir -p {export_folder}/dsx_connect/utils")

    # Copy connectors
    c.run(f"rsync -av --exclude '__pycache__' {project_root_dir}/connectors/sharepoint/ {export_folder}/connectors/sharepoint/ --exclude 'deploy' --exclude 'dist' --exclude 'tasks.py'")
    ...
    c.run(f"rsync -av {project_root_dir}/connectors/sharepoint/deploy/ {export_folder}")
