...
Sharepoint Connector <-*-> DSX-Connect integration.

## Overview

This connector provides and integration for dsx-connect with a Sharepoint.  When this
...
```yaml
      environment:
        - PYTHONUNBUFFERED=1
        - LOG_LEVEL=debug
        - DSXCONNECTOR_CONNECTOR_URL=http://sharepoint-connector:8620 # this should match the aliases below
...
```yaml
      networks:
        dsx-network:
        aliases:
          - sharepoint-connector  # this is how dsx-connect will communicate with this on the network
      command:
        python connectors/Sharepoint/sharepoint_connector.py
```
...
```yaml
services:
  sharepoint_connector_02: # if deploying two of more of this service within a single docker, this name must be unique for each instance
```
to differentiate it from another service like "sharepoint_connector_01"

Next, change "sharepoint-connector" in the following:
```yaml
    environment:
      - DSXCONNECTOR_CONNECTOR_URL=http://sharepoint-connector:8620 # see aliases below
    networks:
      dsx-network:
        aliases:
          - sharepoint-connector
...
```yaml
    environment:
      - DSXCONNECTOR_CONNECTOR_URL=http://sharepoint-connector-02:8620 # see aliases below
    networks:
      dsx-network:
        aliases:
          - sharepoint-connector-02
```
