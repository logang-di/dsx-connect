...
This guide shows how to run the Azure Blob Storage connector with Docker Compose.

## Prerequisites
- Azure Storage credentials (connection string or SAS) with permissions to list/read (and optionally write/move/delete) blobs.
...
Run:
```bash
docker compose -f deploy/docker/docker-compose-azure-blob-storage-connector.yaml up -d
```
