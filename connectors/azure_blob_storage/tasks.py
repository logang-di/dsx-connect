...
project_slug = "azure_blob_storage"
...
def _current_version() -> str:
    # Read without bumping; bump occurs only during release.
...
@task
def clean(c):
    """Remove the local dist export folder and zip for the current version."""
...
@task(pre=[clean])
def prepare(c):
    """Prepare distribution folder with necessary files for the current version."""
    version = _current_version()
    export_folder = _export_folder(version)
...
@task(pre=[prepare])
def zip(c):
    """Zip the prepared export folder."""
    version = _current_version()
    export_folder = _export_folder(version)
...
@task(pre=[zip])
def build(c):
    """Build the Docker image for the current version."""
    version = _current_version()
    export_folder = _export_folder(version)
...
@task(pre=[build])
def push(c):
    """Push the Docker image for the current version to the default repo."""
    version = _current_version()
...
def release(c):
    ...
    # Bump version first so subsequent steps read the new version
...
def helm_package(c, out_dir=None, version=None, app_version=None):
    """Package this connector's Helm chart to a .tgz in out_dir.

    - version/app_version default to the connector's current version.
    ...
    if app_version is None:
        app_version = version
    # Default to dropping the chart into the connector's export folder alongside compose/start.py
    if out_dir is None:
...
def helm_push_oci(c, repo=None, charts_dir=None, version=None):
    """Push the packaged Helm chart to an OCI registry.

    Example: invoke -c connectors.azure_blob_storage.tasks helm-push-oci --repo=oci://registry-1.docker.io/dsxconnect
    """
    if version is None:
        version = _current_version()
    if repo is None:
