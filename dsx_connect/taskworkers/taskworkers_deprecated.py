...
#             if getattr(config.workers, 'retry_connector_connection_errors', True):
#                 backoff_base = getattr(config.workers, 'connector_retry_backoff_base', 60)
...
#             if getattr(config.workers, 'retry_connector_server_errors', True) and retry_count < max_retries:
#                 backoff_base = getattr(config.workers, 'server_error_retry_backoff_base', 30)
...
#             # Client errors
#             if getattr(config.workers, 'retry_connector_client_errors', False):
#                 backoff_base = getattr(config.workers, 'connector_retry_backoff_base', 60)
#                 return _handle_retryable_error(
#                     self, scan_request, e, task_id, retry_count, max_retries,
#                     failure_reason=f"connector HTTP {e.response.status_code} error",
...
#                 backoff_base=getattr(config.workers, "dsxa_retry_backoff_base", 60),
#             )
#
...
#
#         if getattr(config.workers, 'retry_dsxa_connection_errors', True):
#             backoff_base = getattr(config.workers, 'dsxa_retry_backoff_base', 60)
#             return _handle_retryable_error(
#                 self, scan_request, e, task_id, retry_count, max_retries,
...
#
#         if getattr(config.workers, 'retry_dsxa_timeout_errors', True):
#             backoff_base = getattr(config.workers, 'dsxa_retry_backoff_base', 60)
#             return _handle_retryable_error(
#                 self, scan_request, e, task_id, retry_count, max_retries,
...
#             if getattr(config.workers, 'retry_dsxa_server_errors', True):
...
#         # Client errors (4xx)
#         else:
#             if getattr(config.workers, 'retry_dsxa_client_errors', False):
...
#         )
#         dsx_logging.debug(
#             f"Sent verdict for {scan_request.location} to {Queues.VERDICT} with task_id {task1.id}")
#
#     except Exception as e:
#         dsx_logging.error(f"Queue dispatch failed: {e}", exc_info=True)
#
#         # Check config setting for queue dispatch errors
#         if getattr(config.workers, 'retry_queue_dispatch_errors', False):
#             backoff_base = getattr(config.workers, 'dsxa_retry_backoff_base', 60)
#             return _handle_retryable_error(
#                 self, scan_request, e, task_id, retry_count, max_retries,
...
# @celery_app.task(name=Tasks.VERDICT)
...
#     )
#     dsx_logging.debug(
#         f"Sent scan result for {scan_request.location} to {Queues.RESULT} with task_id {task2.id}")
#
#     # 3. Return success response
#     dsx_logging.info(f"Successful {Tasks.VERDICT} processed for {StatusResponse(
#         status=StatusResponseEnum.SUCCESS,
...
# @celery_app.task(name=Tasks.RESULT)
...
#     dsx_logging.info(f"Successful {Tasks.RESULT} processed for {StatusResponse(
#         status=StatusResponseEnum.SUCCESS,
