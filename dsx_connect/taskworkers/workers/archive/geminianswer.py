...
class BaseWorker(Task):
    """
    An abstract base class for Celery tasks that provides standardized
    logging, policy loading, retry, and Dead Letter Queue (DLQ) logic.

    Subclasses must implement the `execute` method, which contains the
    core business logic of the task.
    """
    abstract = True
    # @abc.abstractmethod
    # def execute(self, *args, **kwargs) -> str:
    #     """
    #     The core business logic of the task. This method is called by the
    #     base class's `run` method and is wrapped in error handling.
    #     """
    ...
    def run(self, *args, **kwargs):
        ...
        # Load the appropriate policy for this task execution
        policy = load_policy_variant(policy_override) if policy_override else load_policy()

        # Log initial processing message and policy details on the first attempt
        if retry_count == 0:
            dsx_logging.info(f"[{self.name}:{task_id}] Starting task with policy: {policy.environment}")
            dsx_logging.debug(f"[{self.name}:{task_id}] Policy details: {get_policy_info(policy)}")

        dsx_logging.info(f"[{self.name}:{task_id}] Processing (attempt {retry_count + 1}/{policy.max_retries + 1})")
