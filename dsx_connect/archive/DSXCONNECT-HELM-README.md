...
2.  **global.image:** Optional image defaults inherited by every dsx-connect component.
...
5.  **Worker components:** Scan Request, Verdict Action, Results, Notification, DIANNA (env + Celery concurrency).
...
1) Create the Secret (example manifest provided):
   ...
   - Copy and edit `di-api-secret.yaml` in this directory (set your namespace, API token, and management URL), then apply:
   ...
   kubectl -n <ns> create secret generic di-api \
     ...
     --from-literal=managementUrl="https://di.example.com"
   ```

2) Configure values (preferred) and install:

   Set DIANNA values in a values file (avoid passing secrets on the CLI). Point `global.dianna.secretName` at the Secret you just created and enable the DI worker.
   ...
   global:
     dianna:
       secretName: "di-api"
       # managementUrlKey/apiTokenKey default to managementUrl/apiToken; override only if you renamed keys.
       verifyTls: true
       chunkSize: 4194304
       timeout: 60
...
**2. Create a Custom Values File:**
   ...
   global:
     ...
     dianna:
       secretName: "di-api"                              # Secret created from di-api-secret.yaml
